...
import { 
    ...
    CDropdownItem, CImage , CButton, CPlaceholder, CSpinner , CCardHeader, CModal, CModalHeader, CModalBody, CModalFooter, CBadge, CDropdownDivider
  } from "@coreui/react";
...
const Items = () => {
    ...
    const handleSetStatus = async (itemId, status) => {
        ...
        try {
            ...
            if (response.status === 200) {
                dispatch({ type: 'addAlert', alert: { type: 'success', message: 'Order status updated successfully!' } });
                navigate("/orders");
                window.location.reload();
        }
        } catch (error) {
            dispatch({ type: 'addAlert', alert: { type: 'danger', message: 'Error updating order status!' } });
    ...
    const handleDelete = async () => {
        try {
            ...
            if (response.status === 200) {
                dispatch({ type: 'addAlert', alert: { type: 'success', message: 'Delete order successfully!' } });
        navigate("/orders");
        window.location.reload();
        }
        } catch (error) {
    ...
    <CCard className="mb-2">
        <CCardBody>
            <div className="d-grid gap-2">
                {items.length > 0 ? (
                    <CButton onClick={() => navigate(`/order/new`)} color="primary">+ Add Order</CButton>
                ) : (
                    <CPlaceholder as={CButton} color="primary" aria-hidden="true" disabled href="#" xs={12} />
                )}
            </div>
        </CCardBody>
    </CCard>
                    ...
                    <CTableDataCell>
                      <CDropdown variant="btn-group">
                        ...
                        <CDropdownMenu>
                          ...
                          <CDropdownItem onClick={() => { handleSetStatus(item.id, 2); }}>Paid</CDropdownItem>
                          <CDropdownItem onClick={() => { handleSetStatus(item.id, 5); setShowModal(true); }}>In Transit</CDropdownItem>
                          <CDropdownItem onClick={() => { handleSetStatus(item.id, 6); setShowModal(true); }}>Complete</CDropdownItem>
                        </CDropdownMenu>
                      </CDropdown>
                    </CTableDataCell>
                  </CTableRow>
